...
class BidirectionalSearch extends PathfindingAlgorithm {
  ...
  nextStep() {
    ...
    const currentStart = this.getNextFromOpenSet(
      this.openSetStart,
      this.closedSetStart,
    );
    if (currentStart) {
      ...
      const refEdge = currentStart.edges.find(
        (e) => e.getOtherNode(currentStart) === currentStart.referer,
      );
    ...
    const currentEnd = this.getNextFromOpenSet(
      this.openSetEnd,
      this.closedSetEnd,
    );
    if (currentEnd) {
      ...
      const refEdge = currentEnd.edges.find(
        (e) => e.getOtherNode(currentEnd) === currentEnd.referer,
      );
      if (refEdge) refEdge.visited = true;
