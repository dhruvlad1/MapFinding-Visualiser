...
import { getBoundingBoxFromPolygon, getMapGraph, getNearestNode, getNearestNodeFromGraph } from "../services/MapService";
...
function Map() {
    ...
    async function mapClick(e, info, radius = null) {
        ...
        try {
            ...
            if (loading && (info.rightButton || placeEnd)) {
            ...
            if (info.rightButton || placeEnd) {
                ...
                const node = state.current.graph
                    ? getNearestNodeFromGraph(state.current.graph, e.coordinate[1], e.coordinate[0])
            ...
            ui.current.showSnack("Network error: Could not fetch map data (Overpass API rate limit). Please try again later.");
        } finally {
